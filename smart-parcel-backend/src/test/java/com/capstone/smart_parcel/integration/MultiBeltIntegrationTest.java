...
import com.capstone.smart_parcel.config.security.*;
...
class MultiBeltIntegrationTest extends PostgresIntegrationBase {
    ...
    @Autowired BeltService belts;
    @Autowired ChuteService chutes;
    @Autowired SortingRuleService rules;
    @Autowired DeviceService devices;
    @Autowired StaffAdminService staffAdmin;
    @Autowired DeviceEventService events;
    ...
    Fixture fixture() {
        ...
        long org=jdbc.queryForObject("INSERT INTO public.organizations(code,name) VALUES (?,?) RETURNING id",Long.class,suffix,"Test organization");
        ...
        long belt=belts.createBelt(email,new BeltInput("A","Belt A"));
        long chute=chutes.createChute(email,belt,new ChuteInput("C1","Destination"));
        long version=rules.createVersion(email,belt,input(chute,"Item"));
        var config=rules.publish(email,belt,version);
        belts.activate(email,belt,version,null);
        long rule=config.rules().get(0).id();
        var credential=devices.registerDevice(email,belt,new DeviceInput("device"));
    ...
    EventInput decision(Fixture f) {
        ...
        return new EventInput(UUID.randomUUID(),UUID.randomUUID(),EventType.DECISION,at,f.version,at,at,
                DecisionStatus.MATCHED,InputType.TEXT,"K1S",f.rule,f.chute,null,null);
    }
    ...
    @Test void beltAChangesDoNotAffectBAndOldVersionStillExplainsResults() throws Exception {
        var a=fixture();
        long b=belts.createBelt(a.email,new BeltInput("B","Belt B"));
        long chuteB=chutes.createChute(a.email,b,new ChuteInput("C1","B destination"));
        long vB=rules.createVersion(a.email,b,input(chuteB,"B item")); rules.publish(a.email,b,vB); belts.activate(a.email,b,vB,null);
        long next=rules.createVersion(a.email,a.belt,input(a.chute,"Changed item")); rules.publish(a.email,a.belt,next);
        belts.activate(a.email,a.belt,next,a.version);
        assertEquals(vB,jdbc.queryForObject("SELECT desired_rule_version_id FROM public.conveyor_belts WHERE id=?",Long.class,b));
        var old=decision(a); events.ingest(a.identity(),old,image());
        assertEquals("Item",jdbc.queryForObject("SELECT item_name_snapshot FROM public.sorting_attempts WHERE attempt_id=?",String.class,old.attemptId()));
        assertThrows(Exception.class,()->belts.activate(a.email,a.belt,a.version,a.version));
    }
    @Test void authenticatedApisRejectOtherOrganizationsAndLegacyManagerId() throws Exception {
        var a=fixture(); var b=fixture();
        mvc.perform(get("/api/devices/setup")).andExpect(status().isUnauthorized());
        mvc.perform(get("/api/devices/setup").header("X-Device-Id",a.credential.deviceId()).header("X-Device-Key","wrong"))
                .andExpect(status().isUnauthorized());
        mvc.perform(get("/api/devices/setup").header("X-Device-Id",a.credential.deviceId()).header("X-Device-Key",a.credential.deviceKey()))
        ...
        mvc.perform(post("/api/belts/"+b.belt+"/devices").with(user(a.email)).contentType("application/json")
                .content("{\"code\":\"intruder\"}")).andExpect(status().isNotFound());
        mvc.perform(post("/api/belts/"+a.belt+"/versions").with(user(a.email)).contentType("application/json")
                .content(mapper.writeValueAsBytes(input(b.chute,"Wrong")))).andExpect(status().isNotFound());
        var foreign=decision(b);
        mvc.perform(multipart("/api/devices/events").file(image()).file(new MockMultipartFile("payload","","application/json",mapper.writeValueAsBytes(foreign)))
                .header("X-Device-Id",a.credential.deviceId()).header("X-Device-Key",a.credential.deviceKey()))
                .andExpect(status().isNotFound());
        assertEquals(0L,jdbc.queryForObject("SELECT count(*) FROM public.device_events WHERE event_id=?",Long.class,foreign.eventId()));
    }
    ...
    @Test void errorRetriesNotifyOnceAndRejectUnrelatedChuteFields() throws Exception {
        ...
        assertEquals(1L,jdbc.queryForObject("SELECT count(*) FROM public.user_notifications WHERE event_id=?",Long.class,error.eventId()));
        ...
        mvc.perform(multipart("/api/devices/events").file(new MockMultipartFile("payload","","application/json",mapper.writeValueAsBytes(invalid)))
    ...
    @Test void constraintsRejectWrongOwnerDestinationDraftAndMutation() throws Exception {
        ...
        assertThrows(DataAccessException.class,()->jdbc.update("UPDATE public.sorting_rules SET item_name='tamper' WHERE id=?",a.rule));
        assertThrows(DataAccessException.class,()->jdbc.update("DELETE FROM public.version_chutes WHERE version_id=?",a.version));
        assertThrows(DataAccessException.class,()->jdbc.update("UPDATE public.rule_versions SET status='DRAFT',published_at=NULL WHERE id=?",a.version));
        assertThrows(DataAccessException.class,()->jdbc.update("UPDATE public.sorting_attempts SET chute_id=? WHERE attempt_id=?",b.chute,decision.attemptId()));
        assertThrows(DataAccessException.class,()->jdbc.update("""
                INSERT INTO public.device_events(event_id,organization_id,belt_id,device_id,attempt_id,event_type,error_code,occurred_at,payload_sha256,payload)
                VALUES (?,?,?,?,?,'DEVICE_ERROR','X',now(),?,'{}')
                """,UUID.randomUUID(),a.org,a.belt,b.credential.deviceId(),decision.attemptId(),"a".repeat(64)));
        long draft=rules.createVersion(a.email,a.belt,input(a.chute,"Draft"));
        assertThrows(DataAccessException.class,()->jdbc.update("UPDATE public.conveyor_belts SET desired_rule_version_id=?,desired_set_at=now() WHERE id=?",draft,a.belt));
    }
    @Test void concurrentRetriesCreateOneAttemptImageAndEvent() throws Exception {
        ...
        assertEquals(1L,jdbc.queryForObject("SELECT count(*) FROM public.sorting_attempts WHERE attempt_id=?",Long.class,in.attemptId()));
        assertEquals(1L,jdbc.queryForObject("SELECT count(*) FROM public.event_images WHERE event_id=?",Long.class,in.eventId()));
        ...
        mvc.perform(get("/api/events/"+in.eventId()+"/image").with(user(f.email))).andExpect(status().isOk())
                .andExpect(content().bytes(img.getBytes()));
        var other=fixture();
        mvc.perform(get("/api/events/"+in.eventId()+"/image").with(user(other.email))).andExpect(status().isNotFound());
    }
    @Test void failedWinsInEitherOrderAndDecisionRetryDoesNotResetDischarge() throws Exception {
        ...
        for(boolean reverse:new boolean[]{false,true}) {
            ...
            var row=jdbc.queryForMap("SELECT * FROM public.sorting_attempts WHERE attempt_id=?",in.attemptId());
            ...
            assertEquals(1L,jdbc.queryForObject("SELECT count(*) FROM public.user_notifications WHERE event_id=?",Long.class,no.eventId()));
            assertThrows(DataAccessException.class,()->jdbc.update("UPDATE public.sorting_attempts SET discharge_status='CONFIRMED' WHERE attempt_id=?",in.attemptId()));
        }
    }
    @Test void serverRollbackLeavesNoAttemptEventOrImageAndStaffCannotWrite() throws Exception {
        ...
        assertEquals(0L,jdbc.queryForObject("SELECT count(*) FROM public.sorting_attempts WHERE attempt_id=?",Long.class,in.attemptId()));
        assertEquals(0L,jdbc.queryForObject("SELECT count(*) FROM public.device_events WHERE event_id=?",Long.class,in.eventId()));
        String staff="staff-"+UUID.randomUUID()+"@example.test";
        staffAdmin.createStaff(f.email,new StaffInput(staff,"Staff","password-for-test"));
        mvc.perform(post("/api/belts").with(user(staff)).contentType("application/json").content("{\"code\":\"X\",\"name\":\"X\"}"))
                .andExpect(status().isForbidden());
    }
    @Test void publicationLockBlocksConcurrentChildEdit() throws Exception {
        var f=fixture(); long draft=rules.createVersion(f.email,f.belt,input(f.chute,"Draft"));
        ...
        try {
            var publish=executor.submit(()->new TransactionTemplate(transactionManager).executeWithoutResult(status->{
                jdbc.queryForList("SELECT id FROM public.rule_versions WHERE id=? FOR UPDATE",draft); held.countDown();
                try { if(!release.await(10,TimeUnit.SECONDS)) throw new IllegalStateException("Timeout"); }
                catch(InterruptedException e) { throw new RuntimeException(e); }
                jdbc.update("UPDATE public.rule_versions SET status='PUBLISHED' WHERE id=?",draft);
            }));
            assertTrue(held.await(10,TimeUnit.SECONDS));
            var edit=executor.submit(()->{ editing.countDown(); return jdbc.update("UPDATE public.sorting_rules SET item_name='late' WHERE version_id=?",draft); });
            ...
            assertEquals("Draft",jdbc.queryForObject("SELECT item_name FROM public.sorting_rules WHERE version_id=?",String.class,draft));
    ...
    @Test void pythonSimulatorUploadsRealImageAndErrorThenReplaysSameIds() throws Exception {
        ...
        assertEquals(2L,jdbc.queryForObject("SELECT count(*) FROM public.device_events WHERE device_id=?",Long.class,f.credential.deviceId()));
        assertEquals(1L,jdbc.queryForObject("SELECT count(*) FROM public.event_images i JOIN public.device_events e USING(event_id) WHERE e.device_id=?",Long.class,f.credential.deviceId()));
        UUID event=jdbc.queryForObject("SELECT event_id FROM public.device_events WHERE device_id=? AND event_type='DECISION'",UUID.class,f.credential.deviceId());
    ...
    @Test void jpaHistoryPageAndStatisticsReadTheSameBeltScopedEvents() throws Exception {
        var f = fixture();
        var first = decision(f);
        var second = decision(f);
        events.ingest(f.identity(), first, image());
        events.ingest(f.identity(), second, image());
        var failure = discharge(first, true, 1);
        events.ingest(f.identity(), failure, null);
        String from = "2026-09-24T00:00:00Z", to = "2026-09-25T00:00:00Z";
        mvc.perform(get("/api/attempts").with(user(f.email)).param("beltId", "" + f.belt)
                .param("from", from).param("to", to).param("size", "1"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.content.length()").value(1))
                .andExpect(jsonPath("$.totalElements").value(2))
                .andExpect(jsonPath("$.totalPages").value(2))
                .andExpect(jsonPath("$.content[0].ruleVersionId").value(f.version))
                .andExpect(jsonPath("$.content[0].itemNameSnapshot").value("Item"));
        mvc.perform(get("/api/stats").with(user(f.email)).param("beltId", "" + f.belt)
                .param("from", from).param("to", to)).andExpect(status().isOk())
                .andExpect(jsonPath("$.attempts").value(2)).andExpect(jsonPath("$.matched").value(2))
                .andExpect(jsonPath("$.dischargeFailed").value(1));
        mvc.perform(get("/api/errors/history").with(user(f.email)).param("beltId", "" + f.belt)
                .param("from", from).param("to", to)).andExpect(status().isOk())
                .andExpect(jsonPath("$.totalElements").value(1))
                .andExpect(jsonPath("$.content[0].eventId").value(failure.eventId().toString()));
        var foreign = fixture();
        mvc.perform(get("/api/attempts").with(user(foreign.email)).param("beltId", "" + f.belt)
                .param("from", from).param("to", to)).andExpect(status().isNotFound());
        mvc.perform(get("/api/attempts/" + first.attemptId()).with(user(foreign.email)))
                .andExpect(status().isNotFound());
        mvc.perform(get("/api/attempts").with(user(f.email)).param("beltId", "" + f.belt)
                .param("from", from).param("to", to).param("size", "101")).andExpect(status().isBadRequest());
    }

    @Test void notificationReadAndStaffAdministrationStayInsideOrganization() throws Exception {
        var f = fixture();
        var foreign = fixture();
        String staff = "staff-" + UUID.randomUUID() + "@example.test";
        long staffId = staffAdmin.createStaff(f.email, new StaffInput(staff, "Worker", "password-for-test"));
        var error = new EventInput(UUID.randomUUID(), null, EventType.DEVICE_ERROR, OffsetDateTime.now(),
                null, null, null, null, null, null, null, null, null, "CAMERA");
        events.ingest(f.identity(), error, null);
        assertEquals(2L, jdbc.queryForObject("SELECT count(*) FROM public.user_notifications WHERE event_id=?", Long.class, error.eventId()));
        long notificationId = jdbc.queryForObject("SELECT id FROM public.user_notifications WHERE recipient_user_id=? AND event_id=?", Long.class, staffId, error.eventId());
        mvc.perform(get("/api/notifications").with(user(staff)).param("unreadOnly", "true"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.totalElements").value(1))
                .andExpect(jsonPath("$.data.content[0].eventId").value(error.eventId().toString()));
        mvc.perform(patch("/api/notifications/" + notificationId + "/read").with(user(foreign.email))).andExpect(status().isNotFound());
        mvc.perform(patch("/api/notifications/" + notificationId + "/read").with(user(staff))).andExpect(status().isOk());
        mvc.perform(get("/api/notifications").with(user(staff)).param("unreadOnly", "true"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.totalElements").value(0));
        mvc.perform(delete("/api/admin/staff/" + staffId).with(user(foreign.email))).andExpect(status().isNotFound());
        mvc.perform(delete("/api/admin/staff/" + staffId).with(user(f.email))).andExpect(status().isOk());
        assertFalse(jdbc.queryForObject("SELECT exists(SELECT 1 FROM public.users WHERE id=?)", Boolean.class, staffId));
    }

    @Test void replacementDeviceCanFlushBacklogButRevokedCredentialsNeverBecomeValidAgain() throws Exception {
        var f = fixture();
        devices.deactivateDevice(f.email, f.credential.deviceId(), false);
        var identity = authentication.authenticate(f.credential.deviceId().toString(), f.credential.deviceKey());
        assertNotNull(identity);
        assertFalse(identity.active());
        assertThrows(Exception.class, () -> devices.setup(identity));
        events.ingest(identity, decision(f), image());
        devices.deactivateDevice(f.email, f.credential.deviceId(), true);
        devices.deactivateDevice(f.email, f.credential.deviceId(), false);
        assertNull(authentication.authenticate(f.credential.deviceId().toString(), f.credential.deviceKey()));
        mvc.perform(get("/api/belts/" + f.belt + "/devices").with(user(f.email))).andExpect(status().isOk())
                .andExpect(jsonPath("$[0].credentialHash").doesNotExist())
                .andExpect(jsonPath("$[0].deviceKey").doesNotExist());
    }
