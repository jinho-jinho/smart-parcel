...
import 'sorting_history_detail_screen.dart';

class SortingHistoryScreen extends StatefulWidget {
  const SortingHistoryScreen({super.key});

  @override
  State<SortingHistoryScreen> createState() => _SortingHistoryScreenState();
}

class _SortingHistoryScreenState extends State<SortingHistoryScreen> {
  final _searchCtrl = TextEditingController();
  final List<SortingHistorySummaryDto> _items = [];
  ...
  Future<void> _load({bool reset = false}) async {
    ...
    try {
      final page = await fetchSortingHistory(
        page: _nextPage,
        size: _pageSize,
      ...
      setState(() => _error = '분류 이력을 불러오지 못했습니다.');
    } finally {
      if (reset) {
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return HistoryListRow(
                          ...
                          subtitle: '분류 ID ${item.id}',
                          onTap: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (_) => SortingHistoryDetailScreen(historyId: item.id),
                              ),
                            );
                          },
                        );
